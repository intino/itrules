...
curl -k -c cookies.txt -b cookies.txt --progress-bar -o /dev/null -d "j_username=$usr&j_password=$pwd&submit=" --referer "https://plugins.jetbrains.com/j_spring_security_check" -L https://plugins.jetbrains.com/j_spring_security_check

# check that we have the session cookie, if not, something bad happened, don't spend time uploading.
if [ -z "$(grep JSESSIONID cookies.txt)" ]; then
	cat <<-EOF
...
curl -k -c cookies.txt -b cookies.txt --progress-bar -o /dev/null --referer "https://plugins.jetbrains.com/plugin/uploadPlugin" -L --form pluginId=$plugin_id --form pr=pr --form file=@"$fil" https://plugins.jetbrains.com/plugin/uploadPlugin
